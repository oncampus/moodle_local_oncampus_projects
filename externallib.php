...
syncLog("----------------------------------------------------------------------");
syncLog("instance created rand: ".rand(100,999));
...
public static function get_courseid_parameters() {
   ...
   public static function get_courseid($request) {
    	$params = $request;
    	$courseid = trim($params["coursename"]);
    	syncLog($courseid);
    	...
    	try {  
		    	...
		    	$ret = "none";
		    	global $DB;
		    	 $ret = $DB->get_field('course', 'id', array('idnumber'=>$courseid));
		    	 if (!$ret) {
		    	 	$_courseid = "error";
		    	} else {
		    		$_courseid = $ret;
		    	...
		    	# $ret =  self::inner_get_courseid_by_coursecode($courseid);
    	...
    	return array("courseid" => $_courseid);
    }
     ...
     public static function get_courseid_returns() {
            ...
            new external_single_structure(
                array(
                		'courseid'			 => new external_value(PARAM_TEXT, 'success')
...
public static function isenroled_parameters() {
    ...
    public static function isenroled($request) {
    	...
    	try {
		    	...
		    	$p1 = explode("roleid=",$ret);
		    	$p2 = explode("\">",$p1[0]);
		    	$foundRoleId = $p2[0];
		    	syncLog("extracted roleid = $foundRoleId ");
		    	#$ret = self::enroler_user_courserole($_userid,$_courseid);
		    	syncLog("role found <new> : $ret ");
		    	$status = false;
		    	...
		    	if ($role == 'student' || $role == 'Student' && $ret == 'studierende' or $ret == 'Studierende') {
		    		$status = true;
		    		syncLog("Special: $role found, interpreted as 'Student' ");
		    	}
			} catch (Exception $e) {
				syncLog("Error while parsingincomings arguments!");
				syncLog($e->getMessage());
     ...
     public static function enroler_user_courserole($userid, $courseid) {
    global $CFG, $DB;
 ...
 public static function courseuser_update_parameters() {
  ...
  /**
    ...
    #	$role = $params["courserole"];
    	...
    	if ($role == MOODLE_COURSEROLE_STUDENT) { $role_2 = "student";}
    	if ($role == MOODLE_COURSEROLE_INSTRUCTOR) { $role_2 = "teacher";}
    	if ($role == MOODLE_COURSEROLE_ASSISTANT) { $role_2 = "non-editing-teacher";}
    	if ($role == MOODLE_COURSEROLE_GUEST) { $role_2 = "guest";}
    	...
    	syncLog("checking '$username' in '$courseShort' as '$role_2'?");
	  ...
	  syncLog("lang = ".$buildcourse["lang"]);
		...
		$error='';
				...
				syncLog("received IR:".$restore_ir." for coursetemplate_filetype: ".$restore_coursetemplate_filetype);
									...
									syncLog("mbz  or default ?");
									syncLog("check1 : $check1");
									# syncLog("check2 : $check2 ");
									$found = false;
									$default = $coursetemplatedir."course_default_".$restore_lang.".mbz";
									...
									if ($found == false) {
										...
										synclog("no .mbz found, using default ==> [zip] $default");
										$restore_coursetemplate_filetype = "mbz";
									}
 ...
 # --------------------------------------------------------------------------------------------------------------------------------------------   
    ...
    public static function create_users($users) {
        ...
        $transaction->allow_commit();
				...
				if ($userAllreadyExists == true) { $success = true; }
 ...
 function oc_user_create_user($user) {
  ...
  function syncLog($mess) {
 	$mess = utf8_decode($mess);
 		$handle = fopen( "/opt/www/projects.oncampus.de/moodledata/_syncLog/synclog-".date("Y-m-d",date("U")).".txt" ,"a+");
